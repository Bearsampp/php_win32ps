...
$projectRoot = "c:\projects\php_win32ps"
...
foreach ($arch in $archs) {
    foreach ($ts in $ts_options) {
        ...
        $zipName = "php_win32ps-$PhpVersion-$arch-$ts_label.7z"
        ...
        Get-ChildItem -Path $projectRoot -Include "config.h", "config.nice.bat", "config.log", "config.status", "Makefile", "php_win32ps.res" -Recurse | Remove-Item -Force
